...
INSERT INTO building_info (
    pnu, bld_nm, plat_area, arch_area, bc_rat, vl_rat, tot_area,
...
SELECT DISTINCT ON (pnu)
    ...
    NULLIF(TRIM(b."건축면적"), '')::NUMERIC AS arch_area,
    CASE
    ...
    NULLIF(TRIM(b."연면적"), '')::NUMERIC AS tot_area,
...
WHERE pnu IN (
    SELECT DISTINCT (m.code || CASE WHEN f."대지구분코드명" = '산' THEN '2' ELSE '1' END || LPAD(TRIM(f."주지번"), 4, '0') || LPAD(TRIM(f."부지번"), 4, '0'))
    FROM staging_floor_status f
    JOIN legal_dong_codes m ON (TRIM(f."시군구코드명") || ' ' || TRIM(f."법정동코드명")) = m.name
);
...
SELECT 
    (m.code || CASE WHEN f."대지구분코드명" = '산' THEN '2' ELSE '1' END || LPAD(TRIM(f."주지번"), 4, '0') || LPAD(TRIM(f."부지번"), 4, '0')),
