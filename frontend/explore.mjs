...
const auctionToListing=row=>{
  ...
  const currentFar=(landM2&&bldM2&&landM2>0)?Math.round(bldM2/landM2*100):null;
  ...
  return {id,source:'auction',sourceId:String(row.docid),cohort:'auction',dealType,saleKind:row.sale_kind||'whole',flags:row.flags||null,acquiredRights:row.acquired_rights||null,verifyStatus:row.verify_status||null,verify:row.verify_data||null,
    farLimit,bcrLimit,districtPlan:row.district_plan||'',currentFar,remainingFar,
    ...
    switch(button.dataset.explore) {
      case 'favorite':{
        ...
        button.disabled=true;try{
          ...
          logEvent(active?'favorite':'unfavorite',{source:row.cohort||row.source||'',usage:row.auction?.usageName||row.mainUse||'',district:row.district||'',priceWon:row.priceWon||null,address:row.address||''},row.id);
