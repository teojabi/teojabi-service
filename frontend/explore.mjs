...
// 법원 공시 비고(mulBigo)에서 파생한 위험·특이사항. 위험도 높은 순으로 카드·상세에 배지로 노출한다.
const AUCTION_FLAG_LABELS=Object.freeze([['lien','유치권'],['legalSuperficies','법정지상권'],['landSeparate','토지별도등기'],['unregistered','대지권미등기'],['illegalBuilding','위반건축물'],['saleExcluded','매각제외'],['specialSale','특별매각'],['farmland','농지취득'],['coOwned','공유'],['extraBuilding','제시외']]);
const auctionFlagLabels=row=>{const flags=row.flags||{};return AUCTION_FLAG_LABELS.filter(([key])=>flags[key]).map(([,label])=>label);};
const auctionFlagBadges=(row,limit=4)=>{const labels=auctionFlagLabels(row),shown=labels.slice(0,limit),hidden=labels.length-shown.length;return shown.map(label=>`<em class="pick-badge risk-badge">${esc(label)}</em>`).join('')+(hidden>0?`<em class="pick-badge risk-badge">+${hidden}</em>`:'');};
const auctionRightsBadge=row=>row.acquiredRights?`<em class="pick-badge risk-badge">인수권리</em>`:'';
...
export function mountExplorer(root,{conditions,onEdit,onConditionsChange,onAnalyze,initialId,initialSource,assistant,picksOnly}={}) {
  ...
  function auctionCard(group) {
    ...
    return `<article class="property-card${selected===row.id?' selected':''}" data-card-id="${esc(row.id)}"><button class="property-select" data-explore="detail" data-id="${esc(row.id)}" aria-label="${esc(rowTitle(row))} ${money(row.priceWon)} 상세 보기"><div class="property-location"><span>${esc(rowTitle(row))}</span><em class="pick-badge auction-badge">${badge}</em>${kindBadge}${verifyBadge}${flagBadges}${rightsBadge}${member.get('favorite',row.id)?'<em class="pick-badge favorite-badge">♥ 찜한 물건</em>':''}</div><h2>${money(row.priceWon)}</h2><div class="area-pair"><span>감정가 <b>${money(a.appraisedWon)}</b></span><span>${priceLabel} <b>${money(a.minPrice)}</b></span></div><p class="property-zoning">${esc(a.usageName||'용도 미기재')} · ${esc(row.district||'')} ${esc(row.neighborhood||'')}</p><p class="property-description">${esc(a.caseNo||'')} · ${dateNote}</p><span class="property-link">상세 보기 <span aria-hidden="true">↗</span></span></button></article>`;
  }
