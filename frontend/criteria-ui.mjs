import { ZONING_OPTIONS, AUCTION_USAGES, AUCTION_SOURCES, AUCTION_DEAL_TYPES, parseAreaRange, areaRangeLabel } from './search-options.mjs';
...
export function criteriaFields(values={}) {
  ...
  <fieldset class="criteria-auction"><legend>경매·공매 물건 <small>선택</small></legend><p class="criteria-help">법원경매·공매 물건도 함께 찾아요. 아파트·자동차는 제외됩니다.</p><label class="auction-toggle"><input name="auction" type="checkbox" ${auction.enabled?'checked':''}><span>경매·공매 물건도 함께 보기</span></label><div class="auction-condition-body" ${auction.enabled?'':'hidden'}><div class="auction-row"><span class="auction-row-label">구분</span><div class="zone-choices auction-pill-choices">${AUCTION_SOURCES.map(([value,label])=>`<label><input name="auctionSource" type="radio" value="${value}" ${(auction.source||'court')===value?'checked':''}><span>${label}</span></label>`).join('')}</div></div><div class="auction-row"><span class="auction-row-label">거래 단위</span><div class="zone-choices auction-pill-choices">${[['','전체'],...AUCTION_DEAL_TYPES].map(([value,label])=>`<label><input name="auctionDealType" type="radio" value="${value}" ${(auction.dealType||'')===value?'checked':''}><span>${label}</span></label>`).join('')}</div></div><div class="auction-row"><span class="auction-row-label">매각 구분</span><div class="zone-choices auction-pill-choices">${[['','전체'],['whole','전체 소유'],['share','지분'],['bundle','일괄']].map(([value,label])=>`<label><input name="auctionSaleKind" type="radio" value="${value}" ${(auction.saleKind||'')===value?'checked':''}><span>${label}</span></label>`).join('')}</div></div><div class="auction-row"><span class="auction-row-label">용도 <small>선택하지 않으면 전체</small></span><div class="zone-choices auction-usage-choices">${AUCTION_USAGES.map(u=>`<label><input name="auctionUsage" type="checkbox" value="${esc(u)}" ${usages.includes(u)?'checked':''}><span>${esc(u)}</span></label>`).join('')}</div></div><div class="auction-price-row"><label><span>최저가 이하</span><input name="auctionMaxPrice" type="number" inputmode="decimal" min="0" step="0.1" placeholder="예: 10 (억)" value="${esc(auction.maxPriceEok||'')}" autocomplete="off"></label><label><span>최저가율 이하</span><input name="auctionMaxBidRate" type="number" inputmode="decimal" min="0" max="100" step="1" placeholder="예: 70 (%)" value="${esc(auction.maxBidRate??'')}" autocomplete="off"></label><label><span>유찰 횟수 이하</span><input name="auctionFailMax" type="number" inputmode="numeric" min="0" step="1" placeholder="예: 3 (회)" value="${esc(auction.failMax??'')}" autocomplete="off"></label></div></div></fieldset>`;
}
export function readCriteriaFields(root) {
  return {minArea:root.querySelector('[name="minArea"]').value.trim(),maxArea:root.querySelector('[name="maxArea"]').value.trim(),
    ...
    auction:{enabled:Boolean(root.querySelector('[name="auction"]')?.checked),
      source:root.querySelector('[name="auctionSource"]:checked')?.value||'court',
      dealType:root.querySelector('[name="auctionDealType"]:checked')?.value||'',
      saleKind:root.querySelector('[name="auctionSaleKind"]:checked')?.value||'',
      failMax:root.querySelector('[name="auctionFailMax"]')?.value.trim()||'',
