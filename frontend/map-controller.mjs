...
export class ListingMap {
  ...
  icon(item,active,count,top=false) {
    ...
    element.title=item.cohort==='existing'?'터잡이픽':'선별매물';
    ...
    element.setAttribute('aria-label',`${item.cohort==='existing'?'터잡이픽':'선별매물'} ${item.district} ${item.neighborhood||''} 매물 ${formatPrice(item.priceWon)}, ${area.textContent}, 상세 보기`);
