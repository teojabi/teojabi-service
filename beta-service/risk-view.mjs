...
import { safePublicDocumentUrl, zoneRelationLabel, repKindLabel, TOURISM_NOTICE } from './risk-policy.mjs';
...
function planSources(p) {
  const parts=[];
  if(p.baseNotice?.url)parts.push(`<a class="outline" href="${esc(p.baseNotice.url)}" target="_blank" rel="noopener noreferrer">기준 고시 ${esc(p.baseNotice.no||'')}${p.baseNotice.date?` (${esc(p.baseNotice.date)})`:''} ↗</a>`);
  if(p.representative?.url)parts.push(`<span class="risk-plan-rep"><span class="risk-badge review">${esc(repKindLabel(p.representative.kind))}</span><a href="${esc(p.representative.url)}" target="_blank" rel="noopener noreferrer">${esc(p.representative.name||'대표 자료')}${p.representative.used?' · 값 근거':''} ↗</a></span>`);
  const gl=(p.guidelines||[]).filter(g=>g.url);
  const glHtml=gl.length?`<details class="risk-drawings"><summary>시행지침 ${gl.length}개 살펴보기</summary><ul>${gl.map(g=>`<li><a href="${esc(g.url)}" target="_blank" rel="noopener noreferrer">${esc(g.name||'시행지침')} <span aria-hidden="true">↗</span></a></li>`).join('')}</ul></details>`:'';
  if(!parts.length&&!glHtml)return '';
  return `<div class="risk-plan-sources">${parts.join('')}${glHtml}</div>`;
}
// 지구단위계획 용적률·건폐율·높이 기준 표.
function riskFar(plan) {
  const rows=plan.far||[];
  if(!rows.length)return '';
  const shown=rows.slice(0,15),pct=(v,t)=>v==null?(t||'—'):`${v}%`;
  return `<details class="risk-far" open><summary>지구단위계획 건축 기준 ${rows.length}건 <small>구역 단위 · 획지 적용은 도면 확인</small></summary><div class="risk-far-scroll"><table><thead><tr><th scope="col">구분</th><th scope="col">기준</th><th scope="col">허용</th><th scope="col">상한</th><th scope="col">건폐율</th><th scope="col">높이</th></tr></thead><tbody>${shown.map(i=>`<tr><th scope="row">${esc(i.zoneDetail||i.roadSide||i.zoneClass)}${i.changeType?` <small>${esc(i.changeType)}</small>`:''}</th><td>${esc(pct(i.standard,i.standardText))}</td><td>${esc(pct(i.allowed,i.allowedText))}</td><td>${esc(pct(i.upper,i.upperText))}</td><td>${esc(pct(i.bcr,i.bcrText))}</td><td>${i.heightM!=null?`${i.heightM}m`:i.floors!=null?`${i.floors}층`:'—'}</td></tr>`).join('')}</tbody></table></div>${rows.length>shown.length?`<p class="risk-caption">그 밖에 ${rows.length-shown.length}건이 있습니다.</p>`:''}<p class="risk-caption">원문 고시·도면에서 해당 획지에 적용되는 기준을 확인하세요.</p></details>`;
}
