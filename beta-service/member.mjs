...
function loginChoices(){return `<div class="login-provider-grid" aria-label="소셜 로그인 선택">${[['naver','네이버로 로그인'],['kakao','카카오로 로그인'],['google','구글로 로그인']].map(([provider,label])=>`<a class="outline login-provider" href="${esc(providerLoginUrl(provider))}">${label}</a>`).join('')}</div>`;}
