...
export function mountExplorer(root,{conditions,onEdit,onConditionsChange,onAnalyze,initialId,initialSource,assistant,picksOnly}={}) {
  ...
  function renderAuctionDetail() {
    ...
    $('#listing-detail').innerHTML=`<div class="detail-top"><button type="button" class="detail-back" data-explore="back-list">← 매물 목록</button><button class="detail-close" data-explore="close" aria-label="${onbidMode?'공매':'경매'} 상세 닫기">×</button></div>
      ...
      <div class="detail-conversion"><a class="primary" href="${sourceUrl}" target="_blank" rel="noopener noreferrer">${onbidMode?'온비드 공매 원문 ↗':'법원경매정보 원문 ↗'}</a><button class="outline" data-explore="copy-auction">물건 정보 복사</button><button class="outline" data-explore="favorite" data-id="${esc(row.id)}" aria-pressed="${Boolean(member.get('favorite',row.id))}">${member.get('favorite',row.id)?'♥ 찜함':'♡ 찜하기'}</button><small>${onbidMode?'입찰 전 온비드 공고 원문을 확인하세요.':'입찰 전 법원 원문(매각물건명세서·현황조사서)을 확인하세요.'}</small></div>
